...
export class ZhCn implements Sentence {
    ...
    readonly macVscodeCliLabel: string = 'VS Code 官方 CLI 路径'
    readonly macVscodeCliHelp: string = '请选择应用包内的 Contents/Resources/app/bin/code 文件，例如：/Applications/Visual Studio Code.app/Contents/Resources/app/bin/code。在文件选择器中按 ⌘⇧G，粘贴完整路径后选择 code 文件。不要选择 .app 或 Contents/MacOS/Code，也不要输入 code 命令或附加参数。若使用 /usr/local/bin/code，它必须是指向官方 CLI 的有效符号链接。'
    readonly macVscodeCliInvalid: string = '当前路径不是可用的 VS Code 官方 CLI，请重新选择存在且可执行的 CLI 文件。'
