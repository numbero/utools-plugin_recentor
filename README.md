...
1. 随便某篇笔记选择「复制笔记链接」
2. 获得形如「https://app.yinxiang.com/shard/s48/nl/12021700/xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx」这样的链接, 其中的「12021700」即为用户 ID,
   需要将其填入插件配置中

![evernote user id](https://s4.ax1x.com/2021/12/16/T9IzJe.png)
...
1. 市场版会**拥有开源版没有的功能**；目前计划有两类：**uTools 会员可用功能**，如插件使用统计（已上线），和**付费功能**（目前还没有）；功能的差异化是希望能够丰富 uTools 的付费生态，能够让 uTools
   活得更长久一点；  
...
在 `Setting` 的 Editor 分组启用 Claude Code，然后输入 **`claude`** 进入会话列表。此功能只注册这一个入口指令。列表显示会话标题和完整项目路径，按最近更新时间排序；选中后按回车或点击，即可复制原始 Session ID。复制成功会显示通知，并遵循插件的“执行后立即退出”设置。

数据目录可手动选择；留空时优先使用 uTools 进程可见的 `CLAUDE_CONFIG_DIR`，否则使用用户主目录下的 `.claude`。应选择包含 `projects` 子目录的 Claude 数据根目录，无需配置 Claude 可执行程序。支持 macOS、Windows 和 Linux 的本地目录。

搜索支持项目路径、会话标题，不区分大小写，按空白分词，所有词都须匹配。例如 `my-project 登录` 可分别命中项目路径和标题。该规则仅用于 Claude 入口，不受其他入口的文件路径搜索开关影响。

读取顶层会话 JSONL 并兼容旧版 `sessions-index.json`；仅列出仍有会话文件的主会话，排除后台任务、subagent 和 sidechain。标题优先使用手动命名，其次自动标题、旧索引摘要、首条有效用户文本，最后显示“未命名会话”。项目路径取会话初始 cwd，缺失时使用索引记录，不猜测编码后的目录名。项目目录已删除仍可复制会话 ID。

每次进入会话列表会检查文件变化，搜索时仅过滤已加载数据。损坏行或未写完的末行会跳过；部分文件不可读时提示部分加载失败。此功能不恢复会话、不搜索正文，也不访问远程数据。
